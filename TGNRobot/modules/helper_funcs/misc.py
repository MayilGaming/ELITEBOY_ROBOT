...
def paginate_modules(page_n: int, module_dict: Dict, prefix, chat=None) -> List:
    ...
    if len(pairs) > 8:
        pairs = pairs[modulo_page * 8:8 * (modulo_page + 1)] + [
            (EqInlineKeyboardButton("ᐊ", callback_data="{}_prev({})".format(prefix, modulo_page)),
                EqInlineKeyboardButton("𝙱𝙰𝙲𝙺", callback_data="layla_back"),
             EqInlineKeyboardButton("ᐅ", callback_data="{}_next({})".format(prefix, modulo_page)))]
